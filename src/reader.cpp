    ...
    while (!git_revwalk_next(&oid, walker)) {
        ...
        if(count == 0) {
            ...
            continue ;
              ...
              if(parent_error < 0) {
                  throw runtime_error("Failed to get parent commit");
              }
              ...
              if(commitTree_err < 0) {
                  throw runtime_error("Failed to get commit tree");
              }

              if(parentTree_err < 0) {
                  throw runtime_error("Failed to get parent tree");
              }
              
              
              git_diff *diff = nullptr;
              int diff_error = git_diff_tree_to_tree(&diff, repo, parent_tree, commit_tree, NULL);

              if(diff_error) {
                  throw runtime_error("Failed to get diff between commit and parent trees");
              }
